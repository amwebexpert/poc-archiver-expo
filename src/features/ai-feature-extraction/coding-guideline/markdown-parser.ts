import type { GuidelineNode } from "../models";
...
export const findParentNodeForLevel = ({
  node,
  level,
}: FindParentNodeArgs): GuidelineNode => {
  let currentNode = node;
  while (currentNode.level >= level) {
    if (!currentNode.parent)
      throw new Error(`no parent for level ${level} - ${currentNode.title}`);
...
export const jsonSerializeReplacer = (key: string, value: unknown) => {
  if (["parent", "level", "href", "markdownLines"].includes(key))
    return undefined;
...
export const cloneAndRemoveAllParents = (
  node: GuidelineNode
): GuidelineNode => {
  const newNode = {
    ...node,
...
export const isGuidelineAvoidOrPreferSection = (node: GuidelineNode): boolean =>
  ["❌ avoid", "✅ prefer"].some((section) =>
    node.title.toLowerCase().includes(section)
  );
